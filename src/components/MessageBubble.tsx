...
export default function MessageBubble({ role, content, timestamp, isTyping = false }: MessageBubbleProps) {
          ...
          {lines.map((line, lineIndex) => {
            ...
            if (trimmedLine.startsWith("###") || (lineIndex + 1 < lines.length && lines[lineIndex + 1].includes("====="))) {
              ...
              return (
                <h3
                  key={lineIndex}
                  className="text-lg md:text-xl font-bold text-yellow-800 mb-3 mt-4 first:mt-0 border-b border-yellow-200/50 pb-2"
                >
            ...
            if (trimmedLine.startsWith("##") || trimmedLine.match(/^[A-Z][a-zA-Z\s]*:$/)) {
              ...
              return (
                <h4 key={lineIndex} className="text-base md:text-lg font-semibold text-yellow-700 mb-3 mt-4 first:mt-0">
                  {parseMarkdown(headingText)}
                </h4>
              );
            }
              ...
              return (
                <div key={lineIndex} className="mb-3">
                  <div className="flex items-start space-x-3">
                    <span className="flex-shrink-0 w-6 h-6 md:w-7 md:h-7 bg-yellow-100/80 backdrop-blur-sm text-yellow-800 rounded-full flex items-center justify-center text-sm font-semibold mt-0.5 border border-yellow-200/50">
                      {number}
                    </span>
                    <p className="text-sm md:text-base text-gray-700 leading-relaxed flex-1">{parseMarkdown(listText.trim())}</p>
                  </div>
                </div>
              );
            }
            ...
            if (trimmedLine.match(/^[\*\-•]\s/)) {
              ...
              return (
                <div key={lineIndex} className="mb-2">
                  <div className="flex items-start space-x-3">
                    <div className="flex-shrink-0 w-1.5 h-1.5 md:w-2 md:h-2 bg-yellow-500 rounded-full mt-2.5"></div>
                    <p className="text-sm md:text-base text-gray-700 leading-relaxed flex-1">{parseMarkdown(bulletText.trim())}</p>
                  </div>
                </div>
              );
            }
            ...
            if (trimmedLine.toLowerCase().startsWith("overview:") || trimmedLine.toLowerCase().startsWith("introduction:")) {
              ...
              return (
                <div
                  key={lineIndex}
                  className="mb-4 p-4 bg-yellow-50/80 backdrop-blur-sm rounded-xl border border-yellow-200/50 shadow-sm"
                >
            ...
            // Regular paragraphs
            return (
              <p key={lineIndex} className="text-sm md:text-base text-gray-700 leading-relaxed mb-3 last:mb-0">
  ...
  return (
    ...
    >
      <div
        className={`max-w-[90%] sm:max-w-[85%] md:max-w-2xl lg:max-w-4xl group ${
          isUser
            ? "bg-gradient-to-br from-yellow-400 to-orange-400 text-white rounded-2xl rounded-br-md px-4 py-3 shadow-lg"
            : "bg-white/70 backdrop-blur-xl border border-white/60 rounded-2xl rounded-bl-md px-4 md:px-5 py-3 md:py-4 shadow-lg hover:shadow-xl transition-all duration-200 hover:bg-white/80"
        }`}
      >
        {!isUser && (
          ...
          {isUser ? (
            <div className="flex items-start space-x-3">
              <div className="flex-1">
                <p className="text-sm md:text-base leading-relaxed">{displayedContent}</p>
              </div>
            ...
            <div className="prose prose-sm md:prose-base max-w-none">
              {displayedContent ? (
                <div>{formatContent(displayedContent)}</div>
              ) : (
